<template>
  <q-input ref="field" :model-value="displayValue" :dense="dense" :outlined="outlined" :label="label" reactive-rules
           lazy-rules :rules="rules">
    <template #append>
      <q-btn dense unelevated outline color="secondary" icon="event"
             v-if="['date', 'dateTime'].indexOf(pickerType) !== -1">
        <q-popup-proxy ref="qDateProxy">
          <q-date minimal :model-value="value" @update:model-value="onUpdateValue" :mask="valueFormat"
                  emit-immediately>
            <div class="row items-center justify-end">
              <q-btn v-close-popup label="Закрыть" color="primary" flat/>
            </div>
          </q-date>
        </q-popup-proxy>
      </q-btn>
      <q-btn dense unelevated outline color="secondary" icon="access_time"
             v-if="['time', 'dateTime'].indexOf(pickerType) !== -1">
        <q-popup-proxy ref="qTimeProxy">
          <q-time minimal :model-value="value" @update:model-value="onUpdateValue" :mask="valueFormat" emit-immediately
                  :minute-options="[0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]"
                  format24h>
            <div class="row items-center justify-end">
              <q-btn v-close-popup label="Закрыть" color="primary" flat/>
            </div>
          </q-time>
        </q-popup-proxy>
      </q-btn>
    </template>
  </q-input>
</template>

<script>
import {useFormChild, date} from 'quasar'
import {i18n} from 'boot/i18n'
import {defineComponent} from 'vue'

export default defineComponent({
  name: "CrudDateTime",
  inheritAttrs: false,
  props: {
    rules: {
      type: Array,
      default: () => []
    },
    modelValue: String,
    label: String,
    outlined: {
      type: Boolean,
      default: true
    },
    dense: {
      type: Boolean,
      default: true
    },
    pickerType: {
      type: String,
      validator: function (value) {
        // Значение должно соответствовать одной из этих строк
        return ['date', 'time', 'dateTime'].indexOf(value) !== -1
      },
      default: 'dateTime'
    },

    customDisplayFormat: {
      type: String
    },
    customValueFormat: {
      type: String
    }

  },
  data: () => ({
    value: null
  }),
  computed: {
    valueFormat() {
      //Если в базе dataTime поле хранится экзотически, то берем эту маску:
      if (this.customValueFormat) {
        return this.customValueFormat
      }
      // Маски по стандартным способам хранения даты и времени
      switch (this.pickerType) {
        case 'date':
          return 'YYYY-MM-DD'
        case 'time':
          return 'HH:mm:ss'
      }
      return 'YYYY-MM-DD HH:mm:ss'
    },
    displayValue() {
      let mask = i18n.global.t('maskLocaleDateTime')
      if (this.customDisplayFormat) {
        mask = this.customDisplayFormat
      } else {
        // Маски по стандартным способам хранения даты и времени
        switch (this.pickerType) {
          case 'date':
            mask = i18n.global.t('maskLocaleDate')
            break
          case 'time':
            mask = i18n.global.t('maskLocaleTime')
            break
        }
      }
      return date.formatDate(this.modelValue, mask)
    }
  },
  mounted() {
    useFormChild({validate: this.validate, resetValidation: this.resetValidation, requiresQForm: true})
    this.value = this.modelValue
  },
  methods: {
    onUpdateValue(value, reason, details) {
      this.value = value
      this.$emit('update:model-value', value);
    },
    validate(...args) {
      return this.$refs.field.validate(this.modelValue)
    },
    resetValidation() {
      // console.warn('resetValidation')
    }
  }
})
</script>
